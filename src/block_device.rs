...
pub trait BlockDevice {
    ...
    fn create_partitions(&mut self, boot_size_mib: u64) -> anyhow::Result<PathBuf> {
        ...
        const FIRST_PARTITION_START_SECTOR: u64 = SECTORS_PER_MIB;

        let boot_size_sectors = boot_size_mib * SECTORS_PER_MIB;

        let root_start_sector =
            FIRST_PARTITION_START_SECTOR + boot_size_sectors;
...
label: dos
unit: sectors
start={FIRST_PARTITION_START_SECTOR}, size={boot_size_sectors}, type=c, bootable
start={root_start_sector}, type=83
"
        );

        let mut sfdisk = sudo_cmd("sfdisk")
            .arg(self.path())
            .stdin(Stdio::piped())
            .spawn()?;
